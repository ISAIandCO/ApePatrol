...
flowchart TD
  ...
  B -->|"IndexedDB CRUD"| W["Investigation Workspace"]
...
The built manifest has no `host_permissions` or `content_scripts`. Dynamic registration only follows a granted exact-origin permission and registers only an `ISOLATED` bundle. Internal SIEM API messages are accepted only from a configured SIEM tab, resolve back to that exact origin and pass a method/path allowlist. Generic reads cannot reach mutating Table List routes; those routes require specialized confirmed actions and a second metadata/token check. No key is returned by `settings:get`, profile export, IOC cache or workspace export. External API calls require explicit field action or confirmed batch plus endpoint/data permissions. AI sends only a body whose freshly recomputed SHA-256 matches the exact preview. Managed settings are resolved in background on every load/save and live update.
