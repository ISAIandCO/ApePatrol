...
- [ ] В пошаговом режиме расширение узла добавляет только подтверждённого родителя/непосредственных детей; кандидаты запроса без ребра не появляются изолированными узлами.
...
- [ ] Для 4688/1, 4624/4625 и правил корреляции/нормализации карточки показывают смысловой заголовок; `event_src.title` отображается только как источник события.
...
- [ ] Переключатель «Только общие сущности», масштабирование, перетаскивание узлов, tooltip и «Вписать» работают на светлой и тёмной теме; панель сил изначально свёрнута, настройки и сброс меняют динамическую раскладку.
