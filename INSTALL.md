...
VirusTotal, AbuseIPDB, Kaspersky OpenTIP и ThreatFox включаются независимо. Firefox не позволяет объединять optional-only data-collection consent с host permission, поэтому это намеренно два отдельных пользовательских действия. Для AI также отдельно разрешается точный endpoint; перед отправкой popup показывает endpoint, режим и видимые поля, затем требует подтверждение.
