...
describe("process graph controls", () => {
  it("defaults expansion to 15 minutes and can raise the node limit directly", async () => {
    ...
    const script = await readFile(import.meta.resolve("@isaiandco/ape-share-core/ui/process-graph").replace("file://", ""), "utf8") + await readFile(new URL("../src/process-graph/process-graph.js", import.meta.url), "utf8");
    expect(html).toContain('<option value="900" selected>15 минут</option>');
    expect(html).toContain("Получить до 10 000 событий");
  ...
  it("opens nodes only with the primary button and makes only pinned tooltips interactive", async () => {
    const script = await readFile(import.meta.resolve("@isaiandco/ape-share-core/ui/process-graph").replace("file://", ""), "utf8") + await readFile(new URL("../src/process-graph/process-graph.js", import.meta.url), "utf8");
    const css = await readFile(new URL(import.meta.resolve("@isaiandco/ape-share-core/styles/process-graph.css")), "utf8");
  ...
  it("offers step mode and selective expansion from a pinned node", async () => {
    ...
    expect(script).toContain('expansion("siem:process:expand-node"');
