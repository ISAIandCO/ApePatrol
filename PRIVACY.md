...
На явно разрешённом origin расширение читает DOM карточки события и поля, необходимые для выбранной функции. Настройки UI, instances и provider templates находятся в `storage.local`; ключи VirusTotal, AbuseIPDB, Kaspersky OpenTIP, ThreatFox и LLM — также только в `storage.local`, но под отдельным ключом. Временные versioned snapshots графов находятся в `storage.session` (до десяти последних, максимум 10 000 узлов каждый) и не синхронизируются.
...
SEC AI chat живой SIEM-вкладки хранится в `storage.session` по `tabId`, ограничен 2 MiB и удаляется при закрытии вкладки или Firefox. Постоянный чат Investigation Workspace хранится в отдельном store той же локальной IndexedDB и удаляется вместе с расследованием; в JSON/Markdown export он не входит. Сообщения и очищенные attachments остаются локальными, пока оператор не просмотрит точный payload и не подтвердит внешнюю отправку.
